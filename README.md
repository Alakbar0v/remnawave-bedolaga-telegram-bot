...
Bedolaga поддерживает полный жизненный цикл покупки и активации подарочных подписок как нативно в Telegram-боте, так и через веб-кабинет [Bedolaga Cabinet](https://github.com/BEDOLAGA-DEV/bedolaga-cabinet).

### Руководство для операторов

- **Включение функционала**: Установите параметр `CABINET_GIFT_ENABLED=true` (через `SystemSetting` в базе данных/админ-панели или `.env`). Флаг управляет доступностью каталога подарков одновременно в боте и в веб-кабинете.
- **Настройка тарифов**: Отметьте тарифы, доступные для дарения, флагом `show_in_gift = true` (`SHOW_IN_GIFT`).
- **Синхронизация ссылок**: Убедитесь, что настроены `BOT_USERNAME` (имя Telegram-бота) и `CABINET_URL` (URL веб-кабинета). Бот автоматически формирует канонические deep-link ссылки (`https://t.me/<bot_username>?start=GIFT_<token_prefix>`) и ссылки для веб-активации.
- **Оплата только с баланса**: В Telegram-боте подарок оплачивается строго с внутреннего баланса пользователя. При недостатке средств бот предлагает пополнить баланс через настроенные платежные шлюзы, сохраняя корзину подарка и выполняя автопокупку после пополнения.
- **Модель активации (First-claim ownership)**: Подарок закрепляется за первым пользователем, перешедшим по ссылке (существующим или новым при регистрации через `/start`). Покупатель не может активировать собственный подарок. После успешной активации ссылка становится недействительной для повторных попыток.
- **Бессрочность**: Оплаченные подарки не имеют срока давности и не сгорают со временем.
- **Отключение фичи**: При отключении `CABINET_GIFT_ENABLED=false` каталог подарков скрывается из интерфейса бота и кабинета, однако все ранее созданные и оплаченные подарки сохраняются в базе данных и остаются доступными для активации получателями.

### Cabinet API & Frontend Boundary

Бэкенд предоставляет и тестирует единый контракт API для подарочных подписок:
