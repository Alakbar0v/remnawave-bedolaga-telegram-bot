...
def test_migration_0107_upgrade_downgrade_upgrade_lifecycle():
    """Verify revision custom_0004 upgrade, downgrade, and upgrade on a SQLite database with legacy null rows.

    This migration originally shipped as upstream revision id '0107', but that
    id collided with our fork's own '0107' (guest_purchase_personalized_tgid,
    already deployed in production under that id) and was renamed to
    custom_0004 — see that migration's docstring.
    """
    ...
    migration_path = (
        Path(__file__).resolve().parents[2] / 'migrations/alembic/versions/custom_0004_guest_purchase_idempotency.py'
    )
    spec = importlib.util.spec_from_file_location('migration_custom_0004', migration_path)
