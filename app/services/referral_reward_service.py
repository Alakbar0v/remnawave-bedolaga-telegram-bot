...
#
# Инвариант ledger'а: ``user_id`` — всегда пригласивший, ``referral_id`` — всегда
# приглашённый, независимо от того, кому досталась награда. Менять их местами для
# наград приглашённому нельзя: любой запрос вида «сколько у пользователя рефералов»
# считает DISTINCT referral_id и получил бы в рефералы собственного пригласившего.
# Направление поэтому кодируется причиной, а не перестановкой колонок.
REFEREE_DIRECTED_REASONS = frozenset({REASON_DAYS_REFEREE})


def is_referee_directed(reason: str) -> bool:
    """Награда по этой строке досталась приглашённому, а не владельцу ``user_id``."""
