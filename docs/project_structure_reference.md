...
- `app/cabinet/routes/landing.py` — Python-модуль
  Классы: `LandingFeature`, `LandingTariffPeriod`, `LandingTariff`, `LandingPaymentMethodSubOption`, `LandingPaymentMethod`, `LandingDiscountInfo`, `LandingPersonalization`, `LandingConfigResponse`, `PurchaseRequest` (1 методов), `PurchaseResponse`, `PurchaseStatusResponse`, `GiftClaimRequest` (1 методов), `GiftClaimResponse`
...
- `app/cabinet/routes/tiktok_redirect.py` — Python-модуль
  Классы: нет
  Функции: `tiktok_redirect` — No-JS fallback: capture ttclid and 302 straight to the bot's /start deep link., `tiktok_redirect_json` — JS variant: same capture logic, returned as JSON so the landing page can
...
- `app/database/crud/ticket_notification.py` — Python-модуль
  ...
  Функции: нет
- `app/database/crud/tiktok_click_id.py` — Python-модуль
  Классы: нет
  Функции: `upsert_ttclid` — Insert or update TikTok click id (and optional `_ttp` cookie, client IP,, `get_ttclid` — Get TikTok click id mapping for a user., `mark_registration_sent` — Mark registration event as sent for a user., `mark_trial_sent` — Mark trial event as sent for a user., `mark_first_connected_sent` — Mark first-VPN-connection event as sent for a user.
...
- `app/services/tiktok_events_service.py` — Python-модуль
  Классы: нет
  Функции: `spawn_bg` — Spawn a background TikTok conversion task with proper reference tracking., `fire_registration_bg` — Fire registration event in background with its own DB session., `fire_trial_bg` — Fire trial event in background with its own DB session., `fire_purchase_bg` — Fire purchase event in background with its own DB session., `fire_first_connected_bg` — Fire first-VPN-connection event in background with its own DB session., `store_ttclid` — Store TikTok click id (and optional `_ttp` cookie, client IP, user agent), `store_ttclid_and_fire_registration` — Store ttclid and fire registration conversion in background (best-effort)., `store_ttclid_only` — Persist a freshly-provided ttclid WITHOUT firing a registration event., `on_registration` — Fire registration event (once per user)., `on_trial` — Fire trial-start event (once per user)., `on_first_connected` — Fire first-VPN-connection event (once per user)., `on_purchase` — Fire purchase event (every payment, no dedup)., `resolve_ttclid_token` — Resolve a short redirect token (see tiktok_redirect.py) back to
...
- `app/services/payment/cryptobot.py` — Python-модуль
  Классы: `CryptoBotPaymentMixin` (7 методов)
  Функции: нет
...
- `app/services/payment/platega.py` — Python-модуль
  Классы: `PlategaPaymentMixin` (10 методов)
...
- `migrations/alembic/versions/0106_guest_purchase_campaign.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0107_guest_purchase_personalized_tgid.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
...
- `tests/cabinet/test_landing_personalized_routes.py` — Python-модуль
  Классы: нет
  Функции: `test_landing_routes_registered`, `test_resolve_personalized_user_excludes_blocked_account` — An admin-banned account (User.status = BLOCKED, set by, `test_resolve_personalized_user_excludes_deleted_account`, `test_resolve_personalized_user_allows_active_account`, `test_get_landing_config_without_tgid_has_no_personalization`, `test_get_landing_config_tgid_not_found`, `test_get_landing_config_tgid_has_active_subscription`, `test_get_landing_config_tgid_ok_uses_personal_prices`, `test_purchase_rejects_unknown_tgid`, `test_purchase_rejects_active_subscription`, `test_purchase_stores_numeric_contact_and_user_id` — The purchase record must be bound to the numeric telegram id (not a, `test_purchase_request_validator_rejects_contacts_with_telegram_id`, `test_purchase_request_validator_rejects_gift_with_telegram_id`, `test_purchase_request_validator_requires_contacts_without_telegram_id`, `test_purchase_request_validator_accepts_plain_contacts`, `test_status_response_personalized_hides_subscription_url_and_credentials`, `test_status_response_anonymous_still_exposes_subscription_url` — Regression guard: the anonymous flow's existing within-TTL exposure of
...
- `tests/cabinet/test_tiktok_redirect.py` — Python-модуль
  Классы: нет
  Функции: `test_route_registered`, `test_json_route_registered`, `test_redirect_with_campaign_builds_expected_start_param`, `test_redirect_captures_client_ip_and_user_agent` — IP/UA of the actual ad click must ride along in the cached token blob so, `test_redirect_caches_ttp_alongside_ttclid`, `test_json_variant_returns_same_deep_link_as_redirect` — The background-prefetch endpoint must build the exact same deep link, `test_json_variant_rate_limited_returns_429`, `test_redirect_without_campaign_uses_empty_head_format` — No campaign → `_subid_tt_<token>` (leading underscore = empty head),, `test_redirect_at_the_64_char_boundary_succeeds` — campaign(42) + '_subid_tt_' (10) + token(12) == 64 exactly — must pass., `test_redirect_rejects_campaign_too_long_for_deep_link` — One char over the 64-char boundary must be rejected with a clear 400,, `test_redirect_missing_bot_username_returns_503`, `test_redirect_rate_limited_returns_429`, `test_redirect_rejects_campaign_containing_subid_delimiter`, `test_redirect_survives_cache_failure` — Cache being down must not break the redirect — best-effort write.
...
- `tests/crud/test_user_has_active_subscription.py` — Python-модуль
  Классы: нет
  Функции: `test_user_has_active_subscription_status_boundary`, `test_user_has_active_subscription_false_when_no_rows`, `test_user_has_active_subscription_ignores_other_users`
...
- `tests/database/test_central_purchase_hook_tiktok.py` — Python-модуль
  Классы: нет
  Функции: `tiktok_spy` — Patch both conversion services' hooks plus the other lazy side-effects., `test_completed_subscription_payment_fires_once_with_transaction_id`, `test_deposit_does_not_fire`, `test_not_completed_subscription_payment_does_not_fire_inline`, `test_commit_false_does_not_fire_inline`, `test_negative_stored_amount_fires_positive_abs`, `test_deferred_subscription_payment_fires_once_with_transaction_id`, `test_deferred_deposit_does_not_fire`, `test_single_transaction_does_not_double_fire`
- `tests/database/test_guest_purchase_gift_idempotency.py` — Python-модуль
  Классы: нет
  Функции: `test_guest_purchase_model_has_idempotency_key_column` — GuestPurchase model must have idempotency_key column and ux_guest_purchases_idempotency_key index., `test_multiple_null_idempotency_keys_are_allowed` — Multiple legacy guest purchases with NULL idempotency_key must be allowed., `test_duplicate_non_null_idempotency_key_is_rejected` — Duplicate non-null idempotency_key must trigger uniqueness violation., `test_migration_0107_upgrade_downgrade_upgrade_lifecycle` — Verify revision custom_0004 upgrade, downgrade, and upgrade on a SQLite database with legacy null rows.
...
- `tests/handlers/test_start_ttclid.py` — Python-модуль
  Классы: `TestSplitStartParamSubidForTikTok` (3 методов)
  Функции: `test_drain_fires_registration_for_new_user` — New-registration call sites pass fire_registration=True — the drain must, `test_drain_passes_through_pending_ttp` — The `_ttp` cookie value captured alongside ttclid must ride along into, `test_drain_passes_through_pending_ip_and_user_agent` — The click IP/UA captured at the TikTok ad-click redirect must ride along, `test_drain_stores_only_for_existing_user` — The cmd_start fast-path (user already registered) passes, `test_drain_is_noop_when_no_pending_ttclid`, `test_drain_is_noop_when_get_data_returns_none`, `test_drain_swallows_store_failure` — A DB/network hiccup while storing the ttclid must never crash the, `test_drain_function_imported_from_start_module`
...
- `tests/services/test_guest_purchase_personalized_fulfillment.py` — Python-модуль
  Классы: нет
  Функции: `test_personalized_purchase_skips_find_or_create_user`, `test_personalized_purchase_no_subscription_creates_new`, `test_personalized_purchase_expired_subscription_replaces`, `test_personalized_purchase_never_holds_for_activation` — The race case: the user already has a LIVE subscription by the time the, `test_personalized_purchase_limited_subscription_extends_and_resets_traffic` — LIMITED (out of traffic, days still remaining) is NOT blocked upstream, `test_personalized_purchase_disabled_subscription_replaces_not_extends` — A DISABLED subscription (e.g. channel-membership gate) with a future, `test_personalized_purchase_creates_transaction` — create_transaction must still run for personalized purchases — it, `test_personalized_purchase_consumes_promo_offer` — A one-shot promo offer must be consumed on fulfillment — the landing, `test_personalized_purchase_marks_failed_when_user_vanished` — purchase.user_id and personalized_telegram_id both fail to resolve to a, `test_activate_purchase_rejects_personalized_purchase` — A personalized purchase should never reach PENDING_ACTIVATION, but the
- `tests/services/test_guest_purchase_personalized_pricing.py` — Python-модуль
  Классы: нет
  Функции: `test_personalized_price_matches_pricing_engine` — calculate_personal_tariff_price must reuse PricingEngine verbatim —, `test_personalized_mode_ignores_campaign_discount_entirely` — validate_and_calculate(user=...) must NOT stack the landing's campaign, `test_anonymous_path_unchanged_when_user_is_none` — Regression guard: passing user=None (the default) must keep the exact, `test_personalized_no_discount_returns_no_original_price` — A user with no applicable discount must show no strikethrough price., `test_personalized_hundred_percent_discount_clamps_to_one_kopek` — A 100%-off order must still charge 1 kopek — every payment provider
...
- `tests/services/test_tiktok_events_service.py` — Python-модуль
  Классы: нет
  Функции: `test_is_enabled_requires_all_three`, `test_event_payload_shape`, `test_event_payload_includes_ttp_when_given`, `test_event_payload_omits_ttp_when_invalid`, `test_event_payload_includes_ip_and_user_agent_when_given`, `test_event_payload_omits_ip_when_invalid`, `test_purchase_payload_includes_value_and_currency`, `test_post_event_success_with_code_zero`, `test_post_event_200_with_nonzero_code_is_failure_not_retried` — TikTok can 200 an ill-formed event (e.g. bad pixel) with code != 0 —, `test_post_event_retries_on_5xx_then_succeeds`, `test_post_event_exhausts_retries_on_persistent_5xx`, `test_post_event_4xx_fails_without_retry`, `test_store_and_fire_registration_passes_through`, `test_store_and_fire_registration_noop_without_ttclid`, `test_store_ttclid_only_noop_when_disabled`, `test_on_registration_skips_when_already_sent`, `test_on_registration_skips_without_ttclid_row`, `test_on_registration_fires_and_marks_sent`, `test_on_purchase_fires_every_call_no_dedup_flag` — Unlike registration/trial/first-connected, purchases have no dedup flag —, `test_on_purchase_noop_without_ttclid`, `test_resolve_ttclid_token_reads_cache_key`, `test_resolve_ttclid_token_without_ttp`, `test_resolve_ttclid_token_accepts_legacy_bare_string` — Tokens cached by the pre-ttp version of /cabinet/go/tiktok store a bare, `test_resolve_ttclid_token_returns_none_when_missing`, `test_resolve_ttclid_token_empty_input`
